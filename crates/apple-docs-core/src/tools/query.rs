...
/// Maximum number of detailed documentation entries to fetch
const MAX_DETAILED_DOCS: usize = 3;
...
pub fn definition() -> (ToolDefinition, ToolHandler) {
    (
        ToolDefinition {
            ...
            description:
                "Unified documentation search engine. Send a natural language query and receive \
                 structured context from official documentation. Automatically detects the \
                 appropriate technology (SwiftUI, UIKit, Rust, Telegram Bot API, TON, etc.) \
                 and returns relevant symbols, documentation, code examples, and design guidance. \
                 Use this as your primary entry point for any documentation needs."
                    .to_string(),
            input_schema: json!({
                "type": "object",
                "required": ["query"],
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Natural language query (e.g., 'how to use SwiftUI NavigationStack', 'Rust tokio async runtime', 'Telegram sendMessage API')"
                    },
                    "maxResults": {
                        "type": "number",
            ...
            ProviderType::Apple => {
                ...
                *context.state.framework_index.write().await = None;
                ...
                let capitalized = title
                    ...
                    .unwrap_or_default()
                    + &title[1..];
                let fallback_tech = apple_docs_client::types::Technology {
            ...
            *context.state.framework_index.write().await = None;
            ...
            if let Some(tech) = technologies.get(swiftui_id) {
                *context.state.active_technology.write().await = Some(tech.clone());
                ...
                let fallback = apple_docs_client::types::Technology {
...
async fn search_apple(
    ...
    let mut matches: Vec<(i32, &crate::state::FrameworkIndexEntry)> = index
        ...
        .filter_map(|entry| {
            ...
            let abstract_lower = entry
                ...
                .r#abstract
                .as_ref()
                .map(|a| apple_docs_client::types::extract_text(a).to_lowercase())
                .unwrap_or_default();
    ...
    for (_, entry) in matches.into_iter().take(max_results) {
        ...
        let platforms = entry
            ...
            .platforms
            .as_ref()
            .map(|p| apple_docs_client::types::format_platforms(p));
...
async fn search_rust(
    ...
    let results = items
        ...
        .map(|item| DocResult {
            ...
            related_apis: item.methods.iter().take(5).map(|m| m.name.clone()).collect(),
        })
        .collect();
    ...
    let results = items
        ...
        .map(|item| {
            ...
            DocResult {
                ...
                code_sample: None,
                related_apis: item.fields.iter().take(5).map(|f| f.name.clone()).collect(),
            }
        })
        .collect();
...
async fn search_ton(
    ...
    let results = items
        ...
        .map(|item| DocResult {
            ...
            summary: item.summary.unwrap_or_else(|| item.description.unwrap_or_default()),
            platforms: Some("TON API".to_string()),
            code_sample: None,
            related_apis: item.parameters.iter().take(5).map(|p| p.name.clone()).collect(),
        })
        .collect();
...
async fn search_cocoon(
    ...
    let mut results = Vec::new();
    for section_id in &["architecture", "tdx", "smart-contracts"] {
...
fn extract_code_sample(symbol: &apple_docs_client::types::SymbolData) -> Option<String> {
...
fn build_response(
    ...
    let mut lines = vec![
        markdown::header(1, &format!("📚 Documentation for: {}", intent.raw_query)),
        String::new(),
        markdown::bold("Provider", provider.name()),
        markdown::bold("Technology", technology),
        markdown::bold("Results", &results.len().to_string()),
    ];

    // Query analysis section
    lines.push(String::new());
    lines.push(markdown::header(2, "Query Analysis"));
    lines.push(format!("• **Type:** {:?}", intent.query_type));
    lines.push(format!("• **Keywords:** {}", intent.keywords.join(", ")));

    if results.is_empty() {
        lines.push(String::new());
        lines.push("No results found. Try different keywords or check the technology selection.".to_string());
    } else {
        // Results section
        lines.push(String::new());
        lines.push(markdown::header(2, "Results"));
        lines.push(String::new());
        ...
        for (i, result) in results.iter().enumerate() {
            ...
            if !result.summary.is_empty() {
                lines.push(format!("{}", trim_text(&result.summary, 200)));
            }
            ...
            lines.push(format!(
                "**Path:** `get_documentation {{ \"path\": \"{}\" }}`",
                result.path
            ));

            // Code sample (only for top results)
            if let Some(code) = &result.code_sample {
                if i < MAX_DETAILED_DOCS {
                    lines.push(String::new());
    ...
    // Suggestions section
    lines.push(markdown::header(2, "Next Steps"));
    lines.push("• Use `get_documentation { \"path\": \"...\" }` with any path above to get full details".to_string());
    lines.push("• Use `search_symbols { \"query\": \"...\" }` for more targeted symbol searches".to_string());
    lines.push("• Use `how_do_i { \"task\": \"...\" }` for step-by-step recipes".to_string());
